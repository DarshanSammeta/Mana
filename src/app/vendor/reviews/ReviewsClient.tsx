...
export default function ReviewsClient({ initialReviews }: ReviewsClientProps) {
  ...
  const { data: reviews } = useQuery<ReviewWithDetails[]>({
    ...
    queryFn: () => reviewService.getReviews('VENDOR'),
  ...
  const stats = [
    ...
    { title: "Response Rate", value: reviews?.length ? `${Math.round((reviews.filter((r) => r.vendorResponse).length / reviews.length) * 100)}%` : "0%", icon: MessageSquare, color: "text-emerald-500", bg: "bg-emerald-500/10", trend: "Replied reviews" },
  ];
