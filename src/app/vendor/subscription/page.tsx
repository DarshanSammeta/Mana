...
import { Check, Star, Zap, Crown, ShieldCheck, AlertCircle } from "lucide-react";
  ...
  const upgradeMutation = useMutation({
    mutationFn: async (planId: string) => {
      const orderRes = await axios.post(
        "/api/vendor/subscription/razorpay",
        { planId },
        { headers: { Authorization: `Bearer ${accessToken}` } }
      );
          ...
          handler: async (response: any) => {
            try {
              const verifyRes = await axios.post(
                "/api/vendor/subscription/verify",
                {
                  ...response,
                  planId
                },
    ...
    return (
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map(i => <Skeleton key={i} className="h-[500px] rounded-2xl" />)}
        </div>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-[400px] space-y-4">
        ...
        <h2 className="text-xl font-bold">Failed to load subscription data</h2>
  ...
  return (
    <div className="p-6 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Add more usage cards if needed */}
      </div>
      ...
      <div className="bg-muted/50 border border-border p-6 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-6 grayscale opacity-50">
           {/* Add payment logos if available */}
        </div>
      </div>
    </div>
  );
