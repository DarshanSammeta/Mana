...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  return withErrorHandler(async () => {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await AuditService.logVendorAction(id, "VENDOR_SUSPENDED", {
        id: admin.userId,
        role: admin.role,
    ...
    await NotificationService.triggers.vendorAccountStatus(vendor.userId, "SUSPENDED", reason);
    ...
    logger.info("Vendor suspended", { vendorId: id, adminId: admin.userId, reason });

    return NextResponse.json({ message: "Vendor suspended successfully" });
  }, req);
}
