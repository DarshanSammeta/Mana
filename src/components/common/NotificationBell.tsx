    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className="relative p-2 hover:bg-white/10 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-white/20"
        >
      ...
      <DropdownMenuContent
        align="end"
        className="w-80 p-0 overflow-hidden border-slate-200 rounded-2xl shadow-2xl bg-white/95 backdrop-blur-md"
        onCloseAutoFocus={(e) => e.preventDefault()}
      >
        <div className="bg-slate-50/80 px-4 py-3 border-b flex justify-between items-center">
        ...
        <div className="max-h-[400px] overflow-y-auto scrollbar-hide">
          {isLoading ? (
            ...
            <>
              {notifications.filter(Boolean).map((n: any) => (
                <DropdownMenuItem
                  key={n.id}
                  onClick={() => n && !n.isRead && markAsRead(n.id)}
                  className={cn(
                    "p-4 cursor-pointer border-b last:border-0 block transition-colors outline-none focus:bg-slate-50",
                    n?.isRead ? "opacity-60" : "bg-blue-50/20 hover:bg-blue-50"
                  )}
                >
                  <div className="flex gap-3">
                    <div className={cn("h-8 w-8 rounded-lg flex items-center justify-center shrink-0", getCategoryColor(n?.category || 'SYSTEM'))}>
                      <Bell className="h-4 w-4" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={cn("text-xs mb-0.5", n?.isRead ? "font-semibold" : "font-black")}>{n?.title || 'Notification'}</p>
              ...
              {hasNextPage && (
                <div ref={ref} className="p-4 flex justify-center border-t border-slate-50">
                  {isFetchingNextPage ? (
                    <Loader2 className="h-4 w-4 animate-spin text-primary" />
                  ) : (
                    <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Scroll for more</span>
                  )}
                </div>
              )}
            </>
