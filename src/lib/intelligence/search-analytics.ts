...
export async function trackSearch(data: SearchTrackData) {
  ...
  try {
    ...
    if (query && query.length >= 2) {
      await safeRedis.zincrby(TRENDING_SEARCHES_KEY, 1, query.toLowerCase().trim());
    }

    // 2. Track Category Demand
    if (category) {
      await safeRedis.zincrby(`demand:category:${city || 'global'}`, 1, category);
    }
