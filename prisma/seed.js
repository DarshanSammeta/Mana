...
async function main() {
  console.log("🚀 Starting mega seed process with guaranteed coverage...");
  ...
  console.log("🏪 Creating vendors and service data (Option B)...");
  ...
  const VENDORS_PER_SCOPED_CATEGORY = 2;
  let totalVendorsCreated = 0;

  for (const cat of allCategories) {
    console.log(`   - Generating ${VENDORS_PER_SCOPED_CATEGORY} vendors for ${cat.name} [${cat.eventtype.name}]...`);

    for (let i = 0; i < VENDORS_PER_SCOPED_CATEGORY; i++) {
      ...
      const user = await prisma.user.create({
        data: {
          id: userId,
          email: `vendor_${totalVendorsCreated}@manaevents.in`,
      ...
      const vendorProfile = await prisma.vendorprofile.create({
        data: {
          ...
          description: `Award-winning ${cat.name} services for ${cat.eventtype.name} based in ${city.name}.`,
          city: city.name,
          state: city.state,
          address: `${100 + totalVendorsCreated}, Main Road, ${city.name}`,
          zipCode: "500001",
      ...
      const stIds = cat.subcategory.flatMap(sub => sub.servicetype.map(st => st.id));
      if (stIds.length === 0) continue;

      const serviceTypeId = getRandom(stIds);
      const serviceId = randomUUID();
      const basePrice = BASE_PRICES[cat.name] || 5000;
      ...
      const service = await prisma.service.create({
        data: {
          ...
          title: `${cat.name} - ${cat.eventtype.name} Specialist`,
          description: `Elite ${cat.name} by ${businessName} specialized for ${cat.eventtype.name}.`,
          pricingType: cat.name === "Catering" ? "PER_GUEST" : "PACKAGE",
          basePrice: basePrice,
          updatedAt: new Date(),
        }
      });

      const pkgPromises = PACKAGE_TEMPLATES.map(pkgTemplate => {
        const pkgPrice = basePrice * pkgTemplate.priceMultiplier;
        return prisma.renamedpackage.create({
          data: {
            id: randomUUID(),
      ...
      // Add availability for the next 3 days (Optimized for speed)
      ...
      for (let d = 0; d < 3; d++) {
        ...
        availabilityPromises.push(prisma.availability.create({
          data: {
            id: randomUUID(),
            vendorProfileId: vendorProfile.id,
            date: date,
